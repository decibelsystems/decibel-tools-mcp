...
@decibelsystems/tools must be **self-contained**. All tools use **native file operations** (fs, YAML).
...
|------|-------|-------------|
| **Session Init** | `SessionStart` | Injects context reminding you to run `oracle next_actions`, `voice_inbox_sync`, and `agentic queue_sync` before starting work |
| **Sentinel Pre-Commit** | `PreToolUse` on `git commit` | Agent checks open sentinel issues against the commit — reminds you to close resolved ones |
